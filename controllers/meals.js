...
module.exports.index = async (req, res) => {
    ...
    for (i of meals) {
        if (i.mealName !== 'None') {
            items.push({ mealName: i.mealName, mealType: i.mealType, id: i.id, author: i.author })
        }
    }
...
module.exports.new = async (req, res) => {
    ...
    let items = []
    let ingredients = await Ingredient.find({ author: req.user._id }).populate('author')
    ...
    res.render('meals/new', { catArray, items, mealNames, mealType, title: 'Create A New Meal', page: 'mealsNewEdit' })
}
...
module.exports.create = async (req, res) => {
    ...
    for(let m = 0; m < Object.keys(mealNames).length; m++){
        if(meal.mealName == Object.keys(mealNames)[m] && meal.mealType === Object.values(mealNames)[m]){
            req.flash('error', `The Meal Name: '${meal.mealName}' has already been used, please try again.`);
    ...
    if(meal.newIngredient){
        if(typeof meal.newIngredient.name !== 'string'){
            for(i in meal.newIngredient.name){
    ...
    for(m in userIng){
        if(checkIng.includes(userIng[m].name)){
            req.flash('error', `The Ingredient: '${userIng[m].name}' has already been used, please try again.`);
            return res.redirect('/meals/new');
        }
    }
    ...
    if (meal.newIngredient) {
        if (typeof meal.newIngredient.name == 'string') {
            await Ingredient.create({ name: meal.newIngredient.name, cat: meal.newIngredient.cat, author: req.user._id })
        } else {
            for (let i = 0; i < meal.newIngredient.name.length; i++) {
                await Ingredient.create({ name: meal.newIngredient.name[i], cat: meal.newIngredient.newCat[i], author: req.user._id })
            }
        }
    }
    ...
    if (meal.newIngredient) {
        if (typeof meal.newIngredient.name == 'string') {
            ...
            if (meal.newIngredient.newAddTo === 'W(n)') {
                weekly.push({ qty: meal.newIngredient.newQty, weeklyIngredients: name })
            } else {
                replace.push({ qty: meal.newIngredient.newQty, replaceOnUseIngredients: name })
            }
        } else {
            for (let i = 0; i < meal.newIngredient.name.length; i++) {
                const name = await Ingredient.findOne({
                    $and: [{ name: meal.newIngredient.name[i] }, { author: req.user._id }]
                })
                if (meal.newIngredient.newAddTo[i] === 'W(n)') {
                    weekly.push({ qty: meal.newIngredient.newQty[i], weeklyIngredients: name })
                } else {
                    replace.push({ qty: meal.newIngredient.newQty[i], replaceOnUseIngredients: name })
                }
            }
        }
    }
    ...
    if (meal.ingredient) {
        if (typeof meal.ingredient.name == 'string') {
            ...
            if (meal.ingredient.addTo === 'W') {
                ...
                replace.push({ qty: meal.ingredient.qty, replaceOnUseIngredients: name })
            }
        } else {
            for (let i = 0; i < meal.ingredient.name.length; i++) {
                ...
                if (meal.ingredient.addTo[i] === 'W') {
                    ...
                    replace.push({ qty: meal.ingredient.qty[i], replaceOnUseIngredients: name })
                }
            }
        }
    }
...
module.exports.edit = async (req, res) => {
    ...
    try{
        ...
        res.render('meals/edit', { meal, mealType, catArray, items, title: `Edit ${meal.mealName}`, page: 'mealsNewEdit' });
...
module.exports.update = async (req, res) => {
    ...
    if (meal.newIngredient) {
        if (typeof meal.newIngredient.name == 'string') {
            await Ingredient.create({ name: meal.newIngredient.name, cat: meal.newIngredient.newCat, author: req.user._id })
        } else {
            for (let i = 0; i < meal.newIngredient.name.length; i++) {
                await Ingredient.create({ name: meal.newIngredient.name[i], cat: meal.newIngredient.newCat[i], author: req.user._id })
            }
