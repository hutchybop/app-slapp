...
module.exports.indexCustomise = async(req, res) => {
    ...
    res.render('categories/indexCustomise', {catObj, page: 'categoryCustomise', title: 'Customise Your Categories'})
}
...
module.exports.updateCustomise = async(req, res) => {
    ...
    for(u in newCatsObj){
        if(newCatsObj[u] instanceof Array){
            req.flash('error', 'You cannot have Categories with the same name, please try again.');
    ...
    await Category.findOneAndUpdate({author: req.user.id}, {catList: newCats})
