...
const JoiFlashError = (error, req, res, next, url) => {
    if (error) {
        ...
        if (process.env.NODE_ENV !== "production") { // Allows for generic message in production
            req.flash('error', `${msg}`);
...
// Uses Joi to validate user input for editing an ingredient
module.exports.validateshoppingListMeals = catchAsync(async(req, res, next) => {
...
module.exports.validateshoppingListIngredients = catchAsync(async(req, res, next) => {
